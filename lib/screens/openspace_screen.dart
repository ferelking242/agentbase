...
class _OpenspaceScreenState extends State<OpenspaceScreen> {
  ...
  // ── Download single — sauvegarde dans la galerie native (iOS / Android)
  Future<void> _downloadOne(OpenspaceImage img) async {
    if (!mounted) return;
    showAppSnack(context, 'Téléchargement…', color: kBlue);
    try {
      final bytes = await widget.github.downloadBytes(img.rawUrl);
      await saveToGallery(bytes, img.name, isVideo: img.isVideo);
      if (mounted) {
        showAppSnack(context,
            img.isVideo ? 'Vidéo sauvegardée dans la galerie !' : 'Image sauvegardée dans la galerie !');
      }
    } catch (e) {
      if (mounted) {
        showAppSnack(context, e.toString().replaceAll('Exception: ', '').replaceAll('UnsupportedError: ', ''), isError: true);
      }
    }
  }
  ...
  Future<void> _downloadSelected() async {
    ...
    if (toDownload.isEmpty || !mounted) return;
    showAppSnack(context, 'Sauvegarde de ${toDownload.length} fichier(s)…', color: kBlue);
    int saved = 0;
    ...
    if (mounted) {
      showAppSnack(context, '$saved/${toDownload.length} fichier(s) sauvegardé(s) dans la galerie !');
    ...
    itemBuilder: (_, i) {
      ...
      return GestureDetector(
        ...
        child: Stack(children: [
          _ImageCard(
            image: img,
            httpHeaders: widget.github.downloadHeaders,
        ...
        Expanded(
          child: PageView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              if (item.isVideo) {
                return _VideoPage(url: item.rawUrl, httpHeaders: widget.httpHeaders);
              }
              return InteractiveViewer(
                ...
                child: Center(
                  child: CachedNetworkImage(
                    ...
                    fit: BoxFit.contain,
                    placeholder: (_, __) => const Center(child: CircularProgressIndicator(color: Colors.white38, strokeWidth: 1.5)),
...
// On télécharge le fichier dans le répertoire temporaire avant de lancer le player.
class _VideoPage extends StatefulWidget {
  final String url;
  final Map<String, String> httpHeaders;
  const _VideoPage({required this.url, this.httpHeaders = const {}});
...
class _VideoPageState extends State<_VideoPage> {
  ...
  Future<void> _prepare() async {
    try {
      // makeVideoController handles the platform difference:
      // - Mobile: downloads to temp file (raw.githubusercontent.com has no range requests)
      // - Web:    streams via networkUrl (browser handles range requests natively)
      final ctrl = await makeVideoController(widget.url, widget.httpHeaders);
      await ctrl.initialize();
      ctrl.addListener(() {
        if (mounted) setState(() => _playing = ctrl.value.isPlaying);
      });
      if (mounted) setState(() { _ctrl = ctrl; _initialized = true; _loading = false; });
    } catch (e) {
      if (mounted) setState(() { _error = e.toString().replaceAll('Exception: ', ''); _loading = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(height: 12),
        Text('Chargement vidéo…', style: TextStyle(color: Colors.white38, fontSize: 12)),
      ]));
    }
    if (_error != null) {
    ...
    final ctrl = _ctrl!;
    return GestureDetector(
      onTap: () { _playing ? ctrl.pause() : ctrl.play(); },
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Expanded(
        child: ClipRRect(
          ...
          child: image.isVideo
            ? Container(color: Colors.black87, child: const Center(child: Stack(alignment: Alignment.center, children: [
                Icon(Icons.videocam_rounded, color: Colors.white24, size: 36),
                Positioned(bottom: 6, left: 0, right: 0,
                  child: Icon(Icons.play_circle_outline_rounded, color: Colors.white54, size: 22)),
              ])))
            : CachedNetworkImage(
                imageUrl: image.rawUrl,
                httpHeaders: httpHeaders,
                fit: BoxFit.cover,
                placeholder: (_, __) => Container(color: kCard2, child: const Center(child: CircularProgressIndicator(color: kAccent, strokeWidth: 1.5))),
                errorWidget: (_, __, ___) => Container(color: kCard2, child: const Center(child: Icon(Icons.broken_image_outlined, color: kMuted2, size: 24))),
              ),
        ),
      ),
      Padding(
